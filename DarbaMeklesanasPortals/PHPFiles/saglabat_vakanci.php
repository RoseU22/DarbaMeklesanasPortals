...
if (isset($data['name'], $data['description'], $data['location'], $data['skills'], $data['salary'])) {
    ...
    if (isset($data['id']) && $data['id'] !== null) {
        ...
        $query = "INSERT INTO DMPortals_Vakances (vakances_nosaukums, vakances_apraksts, atrasanas_vieta, nepieciesamas_prasmes, maksa) 
                  VALUES (?, ?, ?, ?, ?)";
        if ($stmt = $savienojums->prepare($query)) {
            $stmt->bind_param("sssss", $data['name'], $data['description'], $data['location'], $data['skills'], $data['salary']);
            if ($stmt->execute()) {
                echo json_encode(['success' => true]);
            } else {
